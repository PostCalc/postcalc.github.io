...
function numToWord(val, divId) {
    ...
    function convert(num) {
        ...
        if (num < 1000) return units[Math.floor(num / 100)] + " Hundred" + (num % 100 !== 0 ? " " + convert(num % 100) : ""); return "";
    }
    let str = "";
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function switchView(viewId, isInsurance = false) {
        viewHome.classList.add('hidden-view'); viewCalc.classList.add('hidden-view'); viewTreasury.classList.add('hidden-view');
    ...
    if(realSelector) realSelector.addEventListener('change', (e) => { toggleInputs(); updateInfoContent(e.target.value); });
    ...
    if (categoryBtns.length > 0) {
        categoryBtns.forEach(btn => {
            btn.addEventListener('click', (e) => {
                categoryBtns.forEach(b => b.classList.remove('active')); e.target.classList.add('active');
                const selectedCategory = e.target.getAttribute('data-cat');
                dropdownItems.forEach(item => {
                    if (item.getAttribute('data-category') === selectedCategory) item.classList.remove('hidden'); else item.classList.add('hidden');
                });
    ...
    if (header && list) {
        ...
        dropdownItems.forEach(item => {
            item.addEventListener('click', () => {
                ...
                dropdownItems.forEach(i => i.classList.remove('selected')); item.classList.add('selected');
        ...
        document.addEventListener('click', (e) => {
            if (!document.getElementById('customSchemeDropdown').contains(e.target)) { list.classList.remove('open'); header.classList.remove('active'); }
        });
    }
    ...
    document.getElementById('btnCalcExtend')?.addEventListener('click', () => {
        ...
        const dStr = document.getElementById('dateOpen').value; const d = dStr ? new Date(dStr) : new Date();
        let extRate = getVal('rdExtRate') || 6.7; let extYrs = parseInt(document.getElementById('rdExtYears').value) || 1;
...
window.setSSAMode = function(mode) {
    document.querySelectorAll('#input-ssa .toggle-btn').forEach(btn => btn.classList.remove('active')); if(window.event && window.event.target) window.event.target.classList.add('active');
    document.getElementById('input-ssa').dataset.mode = mode;
    if (mode === 'annual') { unhide('ssa-annual-inputs'); document.getElementById('ssa-monthly-inputs').classList.add('hidden'); } else { unhide('ssa-monthly-inputs'); document.getElementById('ssa-annual-inputs').classList.add('hidden'); }
};
window.setPPFMode = function(mode) {
    document.querySelectorAll('#input-ppf .toggle-btn').forEach(btn => btn.classList.remove('active')); if(window.event && window.event.target) window.event.target.classList.add('active');
    document.getElementById('input-ppf').dataset.mode = mode;
    if (mode === 'annual') { unhide('ppf-annual-inputs'); document.getElementById('ppf-monthly-inputs').classList.add('hidden'); } else { unhide('ppf-monthly-inputs'); document.getElementById('ppf-annual-inputs').classList.add('hidden'); }
};
window.setMISMode = function(type) {
    document.querySelectorAll('#input-mis .toggle-btn').forEach(btn => btn.classList.remove('active')); if(window.event && window.event.target) window.event.target.classList.add('active');
...
function handleCalculate() {
    ...
    if (s === 'pli' || s === 'rpli') {
        ...
        let gridData = generateInsuranceGrid(p, entryAge, s, d, pMode); if (gridData.length === 0) return showWarn("No actuarial rates available for this age combination.");
